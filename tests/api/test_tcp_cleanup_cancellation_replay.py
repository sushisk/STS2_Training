...
@pytest.mark.asyncio
async def test_decode_cleanup_cancellation_preserves_exact_retry() -> None:
    ...
    assert client.pending_retry == RetryRequest.from_message(request)
    assert client.next_request_seq == 1
