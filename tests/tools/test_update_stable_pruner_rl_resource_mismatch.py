...
def test_resource_quality_mismatch_is_rejected(tmp_path):
    value = record(); value["reward"]["learned_terminal_hp"] = 90.0
    path = tmp_path / "r.jsonl"; path.write_text(json.dumps(value) + "\n", encoding="utf-8")
    with pytest.raises(ValueError, match="learned_resource_quality"):
