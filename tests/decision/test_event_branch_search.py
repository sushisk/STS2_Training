"""Coverage for `best_event_option` (no live server required)."""
...
class _FakeClient:
    def __init__(self, hp_by_action: dict[str, "float | None"], *, fail_action_id: "str | None" = None) -> None:
        self.hp_by_action = hp_by_action
        self.fail_action_id = fail_action_id
        self.emulate_calls: list[dict] = []
        self.cancelled: list[list[str]] = []
        self.released: list[list[str]] = []

    async def emulate_action(
        self, instance_id, parent_branch_id, branch_id, rng_id, decision_point_id, action_id, *, timeout_s
    ):
        self.emulate_calls.append({"branch_id": branch_id, "action_id": action_id})
        if action_id == self.fail_action_id:
            raise RuntimeError("transport failure")
        hp = self.hp_by_action.get(action_id)
        if hp is None:
            return {"status": "faulted"}
        return {"status": "completed", "masked_emulator_dto": {"hp": hp, "boundary": "map_select"}}
...
class BestEventOptionTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_returns_none_with_zero_or_one_event_option_candidates(self) -> None:
        ...
        result_empty = await best_event_option(
            client, instance_id="i", decision_point_id="d", legal_actions=[_CARD], timeout_s=5.0
        )
        result_one = await best_event_option(
            client, instance_id="i", decision_point_id="d", legal_actions=[_OVERCOME, _CARD], timeout_s=5.0
        )
    ...
    async def test_one_candidate_failing_does_not_sink_the_others(self) -> None:
        client = _FakeClient({"a-overcome": 60.0, "a-hold-on": 53.0}, fail_action_id="a-hold-on")
        ...
        self.assertEqual(result, "a-overcome")
        # Cleanup still runs for every branch, including the one whose emulate_action failed.
