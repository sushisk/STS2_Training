"""Coverage for ``best_event_option`` without a live RL server."""
...
class BestEventOptionTest(unittest.IsolatedAsyncioTestCase):
    async def test_prefers_the_option_that_preserves_the_most_hp(self) -> None:
        ...
        self.assertEqual({call["rng_id"] for call in client.emulate_calls}, {1})
    ...
    async def test_returns_none_when_every_evaluated_branch_dies(self) -> None:
        client = _FakeClient({"a-overcome": 0.0, "a-hold-on": -2.0})
    ...
    async def test_incomplete_comparison_falls_back_instead_of_picking_survivor(self) -> None:
        client = _FakeClient({"a-overcome": 60.0, "a-hold-on": None})
    ...
    async def test_candidate_exception_falls_back_and_cleans_created_branches(self) -> None:
        client = _FakeClient(
        ...
        self.assertEqual(client.released, client.cancelled)
