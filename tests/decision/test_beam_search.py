...
class _FakeConnection:
    """Minimal scripted RL stand-in for Beam Search tests.

    `emulate_results` maps `(parent_branch_id, action_id)` to a fixed branch result.
    `emulate_result_sequences` maps the same key to per-attempt results so branch-local
    retry behavior can be exercised while the fake fills physical branch identity from
    each request.
    """
    ...
    def __init__(self) -> None:
        ...
        self.emulate_result_sequences: dict[tuple[str, str], list[dict]] = {}
        self._emulate_result_call_counts: dict[tuple[str, str], int] = {}
        ...
        # a chunk/retry failing after earlier work was admitted); None = never reject.
    ...
    def _next_emulate_result(self, key: tuple[str, str]) -> dict:
        sequence = self.emulate_result_sequences.get(key)
        if sequence is None:
            return self.emulate_results[key]
        if not sequence:
            raise AssertionError(f"empty emulate result sequence for {key!r}")
        call_count = self._emulate_result_call_counts.get(key, 0)
        self._emulate_result_call_counts[key] = call_count + 1
        return sequence[min(call_count, len(sequence) - 1)]
    ...
    async def exchange(self, message: dict, *, deadline: float) -> dict:
        ...
        if operation == "emulate_actions":
            ...
            for item in request["items"]:
                ...
                canned = self._next_emulate_result(key)
...
def _root_decision(
    legal_actions: list[dict], *, decision_point_id: str = "d-root", **dto_extra
) -> dict:
    return {
...
class BeamSearchEngineTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_single_depth_prefers_the_winning_action(self) -> None:
        ...
        result = await engine.search(
            "inst-001", _root_decision(_COMBAT_ACTIONS), timeout_s=5.0
        )
        ...
        self.assertEqual(result.stats.branch_retry_faults, 0)
        self.assertEqual(result.stats.branch_retry_recoveries, 0)

    async def test_retries_only_faulted_candidate_with_fresh_branch_id_and_same_rng(self) -> None:
        client, connection = await self._client()
        connection.emulate_result_sequences = {
            ("root", "strike"): [
                {
                    "status": "faulted",
                    "error": "timeout-1",
        ...
        engine = self._engine(client, max_depth=1, max_branch_attempts=3)
        ...
        self.assertEqual(len(emulate_calls), 3)
        self.assertEqual(
            [[item["action_id"] for item in call["items"]] for call in emulate_calls],
            [["strike", "end"], ["strike"], ["strike"]],
        )
        strike_attempts = [
            item
            for call in emulate_calls
            for item in call["items"]
            if item["action_id"] == "strike"
        ]
        self.assertEqual(len({item["branch_id"] for item in strike_attempts}), 3)
        self.assertEqual({item["parent_branch_id"] for item in strike_attempts}, {"root"})
        self.assertEqual({item["decision_point_id"] for item in strike_attempts}, {"d-root"})
        self.assertEqual(len({item["rng_id"] for item in strike_attempts}), 1)
        self.assertEqual(result.best_root_action_id, "strike")
        ...
        self.assertEqual(result.stats.branch_retry_recoveries, 1)
        self.assertFalse(
            any(isinstance(event, BranchFaultTrace) for event in collector.events)
        )

        created_branch_ids = {
            item["branch_id"] for call in emulate_calls for item in call["items"]
        }
    ...
    async def test_branch_fault_is_counted_only_after_attempt_budget_is_exhausted(self) -> None:
        client, connection = await self._client()
        connection.emulate_results = {
            ("root", "strike"): {
                "status": "faulted",
    ...
    async def test_branch_fault_is_traced_with_its_status_and_fault_kind(self) -> None:
        ...
        fault_events = [
            event for event in collector.events if isinstance(event, BranchFaultTrace)
        ]
        ...
        self.assertEqual(fault.branch_id, strike_attempts[-1]["branch_id"])

    async def test_all_candidates_raise_only_after_local_attempts_are_exhausted(self) -> None:
        client, connection = await self._client()
        single_action = [
            {"action_id": "strike", "action_type": "card", "is_available": True}
        ]
        ...
        with self.assertRaises(AllBranchesFaultedError):
            await engine.search(
                "inst-001", _root_decision(single_action), timeout_s=5.0
            )
        ...
        self.assertEqual(len(emulate_calls), 3)
        attempts = [call["items"][0] for call in emulate_calls]
        self.assertEqual(len({item["branch_id"] for item in attempts}), 3)
        self.assertEqual(len({item["rng_id"] for item in attempts}), 1)
        fault_events = [
            event for event in collector.events if isinstance(event, BranchFaultTrace)
        ]
        self.assertEqual(len(fault_events), 1)
        self.assertEqual(fault_events[0].branch_id, attempts[-1]["branch_id"])
        created_branch_ids = {item["branch_id"] for item in attempts}
        self.assertEqual(set(connection.cancel_calls[0]), created_branch_ids)
        self.assertEqual(set(connection.release_calls[0]), created_branch_ids)

    async def test_retry_round_batch_rejection_is_not_retried_or_traced_as_final_fault(self) -> None:
        client, connection = await self._client()
        connection.emulate_result_sequences = {
            ("root", "strike"): [
                {
        ...
        self.assertEqual([item["action_id"] for item in emulate_calls[1]["items"]], ["strike"])
        self.assertEqual(result.best_root_action_id, "end")
        self.assertTrue(result.reason.startswith("emulate_actions_rejected:"))
        self.assertEqual(result.stats.branches_created, 2)
        self.assertEqual(result.stats.branches_faulted, 0)
        self.assertEqual(result.stats.branch_retry_faults, 1)
        self.assertEqual(result.stats.branch_retry_recoveries, 0)
        self.assertFalse(
            any(isinstance(event, BranchFaultTrace) for event in collector.events)
        )
    ...
    async def test_multi_depth_finds_a_deferred_win(self) -> None:
        ...
        # ("strike"/"end" again) key off PARENT branch_id, not a fixed literal.
        engine = self._engine(client, max_depth=2)
        original_emulate_actions = client.emulate_actions
        depth = {"n": 0}

        async def patched_emulate_actions(
            instance_id, items, *, timeout_s, simulation_options=None
        ):
            depth["n"] += 1
            if depth["n"] == 2:
                for item in items:
    ...
    async def test_splits_wide_frontier_across_max_batch_size(self) -> None:
        ...
        connection.emulate_results = {
            ...
            ("root", "c3"): {
                "status": "completed",
                "decision_point_id": "d-c3",
                "masked_emulator_dto": _alive_dto(),
            },
        }
        engine = self._engine(
            client, max_depth=1, top_k_actions=3, max_batch_size=2
        )

        result = await engine.search(
            "inst-001", _root_decision(three_actions), timeout_s=5.0
        )

        emulate_calls = [
            m for m in connection.messages if m["operation"] == "emulate_actions"
        ]
        self.assertEqual(len(emulate_calls), 2)
    ...
    async def test_chunk_failure_keeps_earlier_chunk_results(self) -> None:
        ...
        self.assertEqual(len(emulate_calls), 2)
        first_chunk_branch_ids = {
            item["branch_id"] for item in emulate_calls[0]["items"]
        }
    ...
    async def test_non_combat_decision_is_not_beam_searched(self) -> None:
        ...
        result = await engine.search(
            "inst-001", _root_decision(non_combat_actions), timeout_s=5.0
        )

        self.assertIsNone(result.best_root_action_id)
        self.assertEqual(result.reason, "not_beam_searchable")
        self.assertEqual(
            [m["operation"] for m in connection.messages], ["start_instance"]
        )
    ...
    async def test_no_legal_actions_short_circuits(self) -> None:
        ...
        result = await engine.search(
            "inst-001",
            _root_decision([], decision_point_id="d-root"),
            timeout_s=5.0,
        )

        self.assertEqual(result.reason, "no_legal_actions")

    def test_max_branch_attempts_must_be_positive(self) -> None:
        with self.assertRaisesRegex(ValueError, "max_branch_attempts"):
            BeamSearchConfig(max_branch_attempts=0)
