...
class LocalProcessTransport:
    ...
    def __init__(
        ...
        *,
        ...
        # Keep the RL repository on sys.path for the transport lifetime. A spawned
        # child imports the pickled target by module name (`API.api_runtime`), so
        # removing this path immediately after the parent import breaks Windows spawn.
    ...
    def call(
        ...
        *,
        ...
        # RLApiServerProcess is synchronous/single-request. The lock also makes the
        # temporary per-call timeout assignment safe if another caller is introduced.
