...
class PlackettLuceLinearStableFrontierPruner(StableFrontierPruner):
    ...
    def select(
        ...
        *,
        ...
        # No actual pruning choice exists when every node survives. Keep the supervised
        # ordering and do not create a zero-information RL step.
        if take == len(frontier):
            return [frontier[index] for index in base_order]
        ...
        return [frontier[index] for index in returned_indices]
